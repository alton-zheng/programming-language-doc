# 基于 keepalived 的 lVS 的搭建

&nbsp;

使用 LVS 过程中，会出现 2 个问题

- 负载均衡器单点故障， 业务下线
- RS 下线，一部分用户请求异常（LVS 还存有这 RS 的负载记录)
...
- × ping 的对象是 ip, ip 是网络层
- √ 访问 -> 底层 http 协议 -> 发请求， 判断返回 200 ok
...
> 它属于第三方应用程序， 不属于 LVS 自带
>
> keepalived 是一个通用的工具，主要作为 HA 实现。可以服务于存在单点故障问题的服务： 
>
> - LVS 
>
> - Nginx
...
- 监听自己服务
- 主通告自己还活着， Backup 监听 Master 状态， 多个 Backup 选举出一个主 LVS 
- 配置： ip 添加 ipvs
- 健康检查 -> RS 
...
![load-balancing-lvs-keepalived](images/load-balancing-lvs-keepalived.png)

&nbsp;

### 配置 keepalived (node01, node04)
...
> 此实验和 DR 模型实验相比，不需要手工配置虚拟网卡信息， keepalived 配制即可
...
> 虽然后续可用 keepalived 命令替代 ipvsadm， ipvsadm 用的场景还挺多： 定位问题等
...
>   - 可以配置多个虚拟服务器
>
> 使用 keepalived 配置，其实就是将 ipvsadm 配置方式自动化并添加健康检查相关功能
>
> 配置之后， 可以发现和之前 ipvsadm 配置方式是类似的
...
>  官方介绍, 也可以使用 linux  `man 5 keepalived.conf`
>
> 查看方式不重要，重要的是学习方法
...
Note:
       ...
       information in order to configure Keepalived. This documenation is sup-
...
DESCRIPTION
       ...
       The  keyword 'include' and variants allow inclusion of other configura-
       tion files from within the main  configuration  file,  or  from  subse-
       quently included files.
       ...
       includew FILENAME - same as include_check wild-card_match
       ...
       include_/etc/keepalived/conf.d/*  could  be  specified, but it
       should not error if there are no files in the directory; in  this  case
       includer  should  be  used.  Otherwise  it  is sensible to use
       includet.

       include handling will not work with if the  include  line  uses  condi-
       tional  configuration or parameter substitution, since the detection of
       the include keywords is done before the processing on conditional  con-
       figuration and parameter substitution.

       The basic include keyword is retained for backward compatibil-
       ity, since it does not produce config errors  if  files  could  not  be
       opened etc.
...
Static routes/addresses/rules
       ...
       (except  shortened  option names are not supported due to ambiguities).
...
Track files
       ...
       the  track  file  are 2147483648 to -2147483648. The value, once multi-
       plied by the weight, will be added to the real server's IPVS weight. If
       the  result  is  <=  than 2147483648 then the checker will be in the
       FAULT state.
...
VRRP instance(s)
       ...
       vrrp_instance <STRING> {
           ...
           garp_extra_if [all] 100 # specifiying 0 disable feature
           ...
           # are resolved by not duplicating virtual_router_ids, the
           # your network switches are probably not functioning correctly.
...
Pre-defined definitions
       ...
       ${_HASH} :  This is replaced by a '#' character, which
       would otherwise start a comment
       ${_BANG} :  This is replaced by a '!' character, which
       would otherwise start a comment
...
Keepalived                        2020-12-18                keepalived.conf(5)
```
...
vrrp_instance <STRING> {
  ...
  virtual_ipaddress {
  192.168.1.110/24 dev eth0 label eth0:3
  }
}

virtual server 192.168.1.110 80 {
  lvs_method DR
  delay_loop 6
  lb_algo rr
  lb_kind DR
...
$ ifconfig 
# 会发现已经创建 ech0:3 的虚拟网卡

$ ipvsadm -ln
TCP 192.168.1.110:80 rr
-> 192.168.1.102:80 Route 1 0 0
-> 192.168.1.103:80 Route 1 0 0
# 可以发现和 ipvsadm 配置方式没啥区别，手动变自动，单车变电车. 手动可以熟悉技能，自动成懒人。哈哈，开个玩笑

# 前端访问后
$ ipvsadm -lnc
# 看到具体的连接信息,具体内容看 lvs 相关文档
```
...
$ service keepalived start
$ ifconfig
# 由于 node04 属于 keepalived backup, master 没宕机， 此台机器的虚拟网卡是不会生效的。

$ ipvsadm -ln
# keepalived 会配置它，其实 ifconfig 没有虚拟网卡信息，这里有没有路由信息，影响不大。
...
此 2 台 RS  机器配置方式和 DR 模型实验保持一致，详情请看 [DR 模型](distributed-load-balancing-dr-model-lvs.md)
...
```bash
# node01 网卡禁用 或 将 keepalived 服务停机
$ ifconfig eth0 down

# 此时， node04,backup 转为 master 
# node04 的虚拟网卡启动
$ ifconfig
```
...
```bash
$ ifconfig eth0 up
# keepalived 权重比较大，up 后，能否转换 master (会考虑 cost, 代价，e.g. 数据包传输情况)
```
...
将 node02 的 httpd 服务关闭，看 keepalived 能否正常的将 node02 从 ln 信息中移除
...
> keepalived 异常关闭时， master backup 异常，会造成数据包发送，接收混乱
>
> 解决单点问题，又引入了新的不可靠的问题
>
> keepalived 可用大数据组件 zookeeper 来解决此问题
>
> 后续会有 zookeeper 的文档更新
